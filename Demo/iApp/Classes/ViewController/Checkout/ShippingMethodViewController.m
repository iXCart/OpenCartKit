...
-(void)prepareTableview
{
    ...
    if (nil == self.bottomView){
        UIView* addNewAddressBar =[[NSBundle mainBundle]loadNibNamed:@"AddCommentBar" owner:self options:0][0];
...
- (void)nextStep:(id)sender
{
    ...
    OCCheckoutService *serivce = [OCCheckoutService setShippingMethod:code comment:comment];
...
- (void)loadData{
    ...
    [serivce execute:^(id response) {
        ...
        _mappingResult =  [Resource parseResponse2Result:response];
         NSDictionary* shipping_methods = [[_mappingResult dictionary] valueForKey :@"shipping_methods"];
